	...
	static Oodle() {
		if (CompressionHelper.EnableLogging) {
			...
			if (NativeMethods.Oodle_CheckVersion(expected, ref version) != 1) {
				Console.WriteLine("Invalid Oodle version! Expected a version compatible with {0} ({1:X8}), got {2} ({3:X8})", ParseOodleVersion(expected), expected, ParseOodleVersion(version), version);
			} else {
				Console.WriteLine("Loaded Oodle Version {0}", ParseOodleVersion(version));
			}
		}
